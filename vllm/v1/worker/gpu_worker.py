...
class Worker(WorkerBase):
    def __init__(
        ...
        self.runner_list: list[GPUModelRunner] = None

    def init_preempted_events(
        self,
        runner_id: int,
        ) -> None:
        preempted_signals = self.vllm_config.parallel_config.preempted_signals
        execution_signals = self.vllm_config.parallel_config.execution_signals
        self.runner_list[runner_id].preempted_signal = preempted_signals[runner_id]
        self.runner_list[runner_id].preempted_signal.set()
        self.runner_list[runner_id].execution_signal = execution_signals[runner_id]
        # self.runner_list[runner_id].execution_signal.set()

        if self.vllm_config.parallel_config.tensor_parallel_size > 1:
            self.runner_list[runner_id].ckpt_epochs = \
                self.vllm_config.parallel_config.ckpt_epochs[runner_id]
            self.runner_list[runner_id].sync_ckpt = \
                self.vllm_config.parallel_config.sync_ckpts[runner_id]
    ...
    def init_runner_groups(self, num_runners) -> list[GPUModelRunner]:
        ...
        with set_current_vllm_config(self.vllm_config):
            self.executors = [ThreadPoolExecutor(max_workers=1) for _ in range(num_runners)]
            self.runner_list = [GPUModelRunner(self.vllm_config, self.model_runner.device, self.model_runner.inputs_embeds) 
                                for _ in range(num_runners)]
            self.vllm_config.skip_kv_init = True
            for i, runner in enumerate(self.runner_list):
                runner.model = self.model_runner.model
                # runner.inputs_embeds = self.model_runner.inputs_embeds
                runner.initialize_kv_cache(self.model_runner.kv_cache_config)
                runner.kv_caches = self.model_runner.kv_caches
                runner.runner_id = i
                self.init_preempted_events(i)
            # if self.vllm_config.parallel_config.tensor_parallel_size > 1:
            for runner in [self.model_runner]+self.runner_list:
                runner.nccl_lock = self.vllm_config.parallel_config.nccl_lock
            with torch.cuda.device(self.model_runner.device):
                torch.cuda.empty_cache()
            logger.info(f"{len(self.runner_list)} runner have been initialized")

            return len(self.runner_list)

    # Get finished kv connector output
    def get_finished(self) -> KVConnectorOutput | None:
        if not has_kv_transfer_group():
            return
        kv_connector = get_kv_transfer_group()
        if kv_connector.connector_worker._reqs_to_process:
            output = KVConnectorOutput()
            output.finished_sending, output.finished_recving = (
                kv_connector.get_finished(None)
            )
            output.invalid_block_ids = kv_connector.get_block_ids_with_load_errors()
            # kv_connector.clear_connector_metadata()
            return output

    # Check if _reqs_to_process is empty
    def if_reqs_to_process(self) -> bool:
        return not get_kv_transfer_group().connector_worker._reqs_to_process
    ...
    def sample_tokens(
        ...
        if virtual_runner is not None:
            return self.runner_list[virtual_runner].sample_tokens(grammar_output)
        else:
